...
실제 휠이 연결된 Windows PC에서 서버를 한 번 실행합니다:

```sh
remote-steer server --token <shared-token>
```

게임이 실행될 Linux PC에서 클라이언트를 한 번 실행합니다:

```sh
remote-steer client <windows-host-or-ip> --token <shared-token>
```

첫 연결이 성공하면 `remote-steer`가 서버 주소와 토큰을 기억합니다. 이후 평소
실행은 이렇게 끝납니다:
...
첫 실행이 성공하면 연결 기본값이 저장됩니다. `REMOTE_STEER_SERVER`,
`REMOTE_STEER_TOKEN`, `REMOTE_STEER_CONFIG`로 값을 덮어쓸 수 있습니다.
